...
- **v2.02** (2026): DBC/BLF整合性チェック機能とデコードエラー処理機能を追加
  - **データ整合性チェック**: 変換前にDBCの定義と実際のBLFデータのデータサイズ不一致を検出する検証機能
  - **デコードエラースキップ**: デコードできないメッセージを自動的にスキップし、有効なデータの処理を継続
  - **エラーサマリー**: 変換終了時に詳細なエラーレポートを表示、どのメッセージIDでデコードエラーが発生し、何件あったかを表示
  - **ユーザー確認**: 整合性の問題が検出された場合、処理を開始する前にユーザーに確認を求める
  - **ユーザーエクスペリエンスの向上**: プログレスインジケーターの改善と、よりクリーンな警告メッセージ
